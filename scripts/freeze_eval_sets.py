#!/usr/bin/env python
"""Generate the evaluation sets once, with a fixed seed, and store them.

    python scripts/freeze_eval_sets.py [--config configs/base.yaml] [--force]
    python scripts/freeze_eval_sets.py --task enhance
...
Writes ``data/frozen/<task>/<split>/`` for the train, validation and test buckets
of each task. Per task, because the corner detector's samples include coloured
page stock and pages that will not lie flat, and the enhancement network cannot
be scored against a clean target on either of those.
...
def main() -> int:
    ...
    parser.add_argument("--task", nargs="*", default=None, help="enhance, corner (default: both)")
