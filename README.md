...
Heatmap regression, scored in the detector's own 256×256 input space. The baseline is a classical
Canny + `findContours` + `approxPolyDP` detector on the identical input — the line a learned detector
has to clear to be worth its parameters.
...
About one pixel, and all four corners inside 2% of the diagonal on 95.5% of unseen photos. PCK is
reported beside the mean because the two detectors have opposite error profiles: the classical one is
near-exact when it finds a page and returns nothing otherwise, so a mean flatters the network and a
success rate flatters the baseline. On the 19 real phone photos, **17 came straight from the network**
— the other two went to the guardrail, which is what it is for.

Coordinate regression is training now; the head-to-head verdict, and the check against the prediction
in [`reports/PREDICTIONS.md`](reports/PREDICTIONS.md), lands with it.
